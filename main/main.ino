...
void setup()
{
  ...
  { // วนซ้ำจนกว่าจะเชื่อมต่อ Wifi สำเร็จ
    delay(500);
    Serial.print(".");
  }
...
{ // เริ่ม Loop Main Function
  ...
  if (currentMillisedcond - previousMillis >= interval)
  {
    ...
    {                                                                                                           // ทำการเชื่อมต่อ API โดยใช้ตัวแปร server_ip และ SERVER_PORT
    ...
    { // ตัวสอบเงื่อนไขว่าค่ากระแสที่อ่านได้เป็นตัวเลขใช้หรือไม่ [Note:NAN คือ Not a Number]
      ...
      { // หากฟังก์ชัน check_safety(power) ส่งคืนค่า false กลับมาแสดงว่ากำลังไฟที่ใช้งานอยู่ยังไม่เกินกว่าที่ตั้งไว้หรือไม่ได้เปิดใช้งานการจำกัดกำลังไฟ
        ...
        for (int i = 0; i < 4; i++)
        {
          ...
          if (!doc_channel["data"][i]["templatestatus"])
          { // ทำการตรวจสอบเงื่อนไขว่า ปลั๊กตัวที่ 1 ถึง 4 อันใดมีการตั้งเวลาการทำงานไว้หรือไม่โดยจะตรวจสอบจากตัวแปร
          ...
          {                                                                                                   // หากตรวจสอบแล้ว มีการตั้งเวลากับปลั๊กไฟตั้งแต่ 1 ถึง 4 จะทำการทำงานตามเวลาที่ตั้งไว้ เฉพาะตัวที่มีการตั้งเวลาเท่านั้น
    ...
    {                                               // หากอ่านค่าของ พลังงานไม่ได้ จะไม่สามารถสั่งงานใดๆได้เลย
    ...
    if (etime == now.minute())
    { //(millis() - previousMillis1 >= 60000) {  //ตรวจสอบเงื่อนไขว่า วันที่ปัจจุบันตรงกับวันที่ตั้งค่าไว้หรือไม่และในวันนั้นมีการส่งค่าไปยังฐานข้อมูลหรือยังโดยอธิบายได้ดังนี้
      ...
      {                                     // หากค่าที่อ่านได้จาก energy น้อยกว่า 0 จะทำตามคำสั่งภายในเงื่อนไข else
...
void CONTROL_PLUG_ON_OFF(int channelId, bool status)
{
  ...
  switch (channelId)
  { // switch ฟังก์ชันคือการตรวจสอบค่าที่ต้องการ ในที่นี้คือ channelID โดยจะมีเงื่อนไขดังนี้
...
void time_plug_on_off(int channelId, int templateId, DateTime now)
{
  ...
  if (doc_template["data"]["type"])
  { // type is 1 mean time set
    ...
    Serial.println(String("Date : ") + dayStart + "/" + monthStart + "/" + yearStart + " -> " + dayEnd + "/" + monthEnd + "/" + yearEnd);
    if (now.year() >= yearStart && now.year() <= yearEnd && now.month() >= monthStart && now.month() <= monthEnd && now.day() >= dayStart && now.day() <= dayEnd)
    {
      ...
      Serial.println(String("Time  ") + hour_start + ":" + minute_start + " -> " + hour_end + ":" + minute_end);
      ...
      {                                        // หากไม่เข้าเงื่อนไขที่ว่า `เวลาปัจจุบันไม่อยู่ในช่วงที่ตั้งเวลาไว้`
  ...
  { // type is 0 mean weekly set
    ...
    int weekEnd = doc_template["data"]["daystart"];
    Serial.println("Templete[Weekly Set] Detail : ");
    Serial.println(String("Day : ") + dayofWeek[weekStart] + " to " + dayofWeek[weekEnd]);
...
{ // ฟังก์ชันที่มีไว้สำหรับการส่งข้อมูลไปยังฐานข้อมูล โดยผ่าน API
  ...
  if (httpCode > 0)
  {                                     // หากมีการส่งคืนค่า httpCode คืนมาหมายความว่าฟังก์ชันติดต่อทำงานได้และสามารถเชื่อมต่อกับ Internet ได้ ซึ่งในอีกความหมายคือสามารถส่งข้อมูลไปทาง API ได้อย่างไม่มีปัญหา
  ...
  {                                                 // หากไม่สามารถส่งค่าขึ้นไปหรือติดต่อกับ api ไม่ได้จะแจ้งเตือน
...
bool check_safety(float power)
{
  ...
  if (!client.connect(server_ip, SERVER_PORT))
  {                                                                                                               // ตรวจสอบการเชื่อมต่อกับ API ว่าทำการเชื่อมต่อได้หรือไม่ โดยจะเชื่อมต่อกับ server_ip (ip หรือ domain name ที่ได้ประกาศไว้) และ SERVER_PORT (port การเชื่อมต่อ เช่น 8888)
  ...
  if (power > doc_savety["data"]["volt"] && doc_savety["data"]["status"])
  { // เงื่อนไขสำหรับตรวจสอบว่า ค่าพลังงานไรวงจรเกินกว่าที่ตั้งค่าไว้ไหม และ ได้ทำการเปิดใช้งานการจำกัดพลังงานไหม
  ...
  { // หากพลังงานที่ตั้งไว้ไม่เกิน หรือไม่ได้เปิดการจำกัดไว้ จะเข้าเงื่อนไข else
    Serial.println("");
